...
import { Sparkles, Trash2, Maximize2, X, MessageCircle } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import { SavedTryOn, ClothingItem, UserPhoto } from '../types';
...
export const TryOnGallery = ({ autoOpenImageUrl, onAutoOpenComplete }: TryOnGalleryProps = {}) => {
  ...
  const handleChatClick = async (tryOn: SavedTryOn, e: React.MouseEvent) => {
    e.stopPropagation();

    // Fetch user photo and clothing items for chat context
    try {
      const [userPhotos, clothingItems] = await Promise.all([
        storageService.getUserPhotos(),
        storageService.getClothingItems(),
      ]);

      const userPhoto = userPhotos.find((p) => p.id === tryOn.user_photo_id);
      const clothingPhotos = clothingItems.filter((item) =>
        tryOn.clothing_item_ids.includes(item.id)
      );
      ...
      if (userPhoto) {
        setChatPanelData({
          ...
          clothingPhotosUrls: clothingPhotos.map((item) => item.imageUrl),
        });
  ...
  return (
    <div className="space-y-8">
          ...
          {savedTryOns.map((tryOn) => (
            ...
            >
              <div className="aspect-[3/4] relative">
                ...
                <button
                  onClick={(e) => handleChatClick(tryOn, e)}
                  className="absolute top-3 left-3 p-2 bg-purple-500/90 backdrop-blur-sm rounded-full shadow-lg opacity-0 group-hover:opacity-100 transition-opacity hover:bg-purple-600"
                >
      ...
      {/* Chat Panel */}
      {showChatPanel && chatPanelData && (
        <OutfitChatPanel
          tryOn={chatPanelData.tryOn}
          userPhotoUrl={chatPanelData.userPhotoUrl}
          clothingPhotosUrls={chatPanelData.clothingPhotosUrls}
          onClose={() => {
            setShowChatPanel(false);
            setChatPanelData(null);
          }}
        />
      )}
    </div>
